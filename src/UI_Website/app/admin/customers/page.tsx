...
import { Search, MoreVertical, Mail, Phone, Ban, CheckCircle2, Trash2, Shield, ShoppingBag, X, Store, CreditCard } from "lucide-react"
        ...
        <div className="space-y-8 font-sans">
            <div className="flex justify-between items-center">
                <div className="space-y-1">
                    <h3 className="text-2xl font-black text-gray-900 tracking-tight">Quản lý Khách hàng</h3>
                    <p className="text-xs text-gray-400 font-bold uppercase tracking-widest">Quản lý tài khoản và phân quyền người dùng</p>
                </div>
            ...
            <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
                <table className="w-full text-left">
                    <thead className="bg-gray-50 border-b border-gray-100">
                        <tr>
                            <th className="px-6 py-4 text-xs font-black text-gray-400 uppercase tracking-widest">Khách hàng</th>
                            <th className="px-6 py-4 text-xs font-black text-gray-400 uppercase tracking-widest">Liên hệ</th>
                            ...
                            <th className="px-6 py-4 text-xs font-black text-gray-400 uppercase tracking-widest">Tác vụ</th>
                        </tr>
                    </thead>
                                ...
                                <td className="px-6 py-5">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-gray-400 font-medium italic">#{user.id}</p>
                                        </div>
                                    </div>
                                </td>
                                <td className="px-6 py-5">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <Dialog>
                                            <DialogTrigger asChild>
                                                <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-blue-50 hover:text-blue-500 rounded-lg" title="Xem đơn hàng">
                                            ...
                                            <DialogContent className="sm:max-w-[425px]">
                                                ...
                                                <div className="py-4 space-y-3">
                                                    {user.orderCount > 0 ? (
                                                        [...Array(Math.min(3, user.orderCount))].map((_, idx) => (
                                                            <div key={idx} className="p-3 border border-gray-100 rounded-xl space-y-1">
                                                                <div className="flex justify-between items-center text-sm">
                                                                    <span className="font-bold">Đơn hàng #ORD{2024 + idx}</span>
                                                                    <span className="text-green-500 font-bold">45.000₫</span>
                                                                </div>
                                                                <div className="flex justify-between items-center text-xs text-gray-400">
                                                                    <span>20/02/2026</span>
                                                                    <div className="flex items-center gap-1.5 text-blue-500 font-bold">
                                                                        <CreditCard className="h-3 w-3" />
                                                                        <span>{idx % 2 === 0 ? "Ví SmartBite" : "Tiền mặt"}</span>
                                                                    </div>
                                                                </div>
                                                                <div className="flex justify-end pt-1">
                                                                    <span className="px-2 py-0.5 bg-gray-100 rounded-full text-[10px] text-gray-400 font-bold uppercase">Hoàn thành</span>
                                                                </div>
                                                            </div>
                                                        ))
                                                    ) : (
                                                    ...
                                                    {user.orderCount > 3 && (
                                                        <p className="text-center text-xs text-gray-400">Và {user.orderCount - 3} đơn hàng khác...</p>
                                                    )}
                                                </div>
                                            </DialogContent>
                                        </Dialog>
