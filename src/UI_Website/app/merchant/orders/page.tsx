    ...
    async function fetchOrdersFromServer() {
        try {
            ...
            if (response.ok) {
                ...
                const formattedOrders = rawOrders.map(function (order: any, idx: number) {
                    const statusLabel = getStatusDisplayName(order.orderstatus);


                    const paymentRecord = Array.isArray(order.payments)
                        ? (order.payments.length > 0 ? order.payments[0] : null)
                        : order.payments;
                    ...
                    return {
                        ...
                        orderItemsSummary: (order.orderitems || []).map(function (item: any) {
                            const toppingList = item.orderitemtoppings?.map(function (t: any) {
                                return t.toppingoptions?.toppingname;
                            }).filter(Boolean).join(", ");

                            return item.fooditems?.foodname + (toppingList ? ` (+${toppingList})` : "");
                        }).filter(Boolean).join(", ") || "No items",
                        totalPriceFormatted: new Intl.NumberFormat('vi-VN').format(order.finalamount || 0) + 'đ',
                        orderTimeFormatted: new Date(order.ordertime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
                        currentStatus: statusLabel,
                        currentPaymentStatus: paymentStatus,
                        paymentMethod: paymentRecord?.paymentmethod || (idx % 2 === 0 ? "Ví SmartBite" : "Tiền mặt"),
            ...
            if (response.ok) {
                ...
                setOrderList(function (previousList) {
                    return previousList.map(function (order) {
                        return order.id === orderId ? { ...order, currentStatus: targetStatus } : order;
                    });
                });
        ...
        <div className="space-y-8 max-w-6xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
                <div>
                    ...
                    <p className="text-gray-500 mt-1">Quản lý đơn hàng đến và theo dõi tiến độ giao hàng</p>
                </div>
            </div>
                            ...
                            {paginatedOrders.map(function (order) {
                                return (
                                    <tr key={order.id} className="hover:bg-gray-50/50 transition-colors">
                                        ...
                                        <td className="px-6 py-5">
                                            <span className="text-sm text-gray-600 line-clamp-1">{order.orderItemsSummary}</span>
                                        </td>
                                        ...
                                        <td className="px-6 py-5">
                                            <span className="font-bold text-gray-900">{order.totalPriceFormatted}</span>
                                        </td>
                                        ...
                                        <td className="px-6 py-5">
                                            {order.currentStatus === 'pending' && (
                                                <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ${order.currentPaymentStatus === 'completed'
                                                    ? 'bg-green-100 text-green-700 border border-green-200'
                                                    : 'bg-gray-100 text-gray-700'
                                                    }`}>
                                                    {order.currentPaymentStatus === 'completed' ? 'Đã thanh toán - Xác nhận ngay' : 'Chờ thanh toán'}
                                                </span>
                                            )}
                                        ...
                                        <td className="px-6 py-5 text-center">
                                            {order.currentStatus === 'pending' && (
                                                <button
                                                    onClick={function () { changeOrderStatus(order.id, 'accepted'); }}
                                            ...
                                            {order.currentStatus === 'accepted' && (
                                                <button
                                                    onClick={function () { changeOrderStatus(order.id, 'preparing'); }}
                                            ...
                                            {order.currentStatus === 'preparing' && (
                                                <button
                                                    onClick={function () { changeOrderStatus(order.id, 'delivering'); }}
                                            ...
                                            {order.currentStatus === 'delivering' && (
                                                <button
                                                    onClick={function () { changeOrderStatus(order.id, 'completed'); }}
