...
export default function AdminReviews() {
    ...
    function getFilteredReviews() {
        return reviewList.filter(function (review) {
            ...
            const isTypeMatch = activeTypeFilter === "all" || review.type === activeTypeFilter;
                ...
                <div className="p-6 border-b border-gray-100 space-y-4">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div className="flex flex-wrap items-center gap-2">
                            <button
                                onClick={function () { setActiveRatingFilter(null); }}
                            ...
                            {[5, 4, 3, 2, 1].map(function (stars) {
                                return (
                                    <button
                                        key={stars}
                                        onClick={function () { setActiveRatingFilter(stars); }}
                    ...
                    <div className="flex items-center gap-4">
                        <span className="text-sm font-bold text-gray-400">Loại đánh giá:</span>
                        <div className="flex gap-2">
                            <button
                                onClick={function () { setActiveTypeFilter("all"); }}
                                className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${activeTypeFilter === "all" ? "bg-gray-900 text-white shadow-md" : "bg-gray-100 text-gray-500 hover:bg-gray-200"}`}
                            >
                                Tất cả
                            </button>
                            <button
                                onClick={function () { setActiveTypeFilter("order"); }}
                            ...
                            <button
                                onClick={function () { setActiveTypeFilter("food"); }}
                                className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${activeTypeFilter === "food" ? "bg-green-600 text-white shadow-md" : "bg-green-50 text-green-600 hover:bg-green-100"}`}
                            >
                                Món ăn
                            </button>
                        </div>
                ...
                <div className="divide-y divide-gray-50">
                    {isPageLoading ? (
                        ...
                        <div className="p-16 text-center text-gray-500">
                            ...
                            <p className="font-medium">Chưa có đánh giá nào phù hợp với bộ lọc.</p>
                        </div>
                    ) : (
                                        ...
                                        <div className="w-full lg:w-1/4 space-y-5 border-r-0 lg:border-r border-gray-100 pr-0 lg:pr-8">
                                            ...
                                            <div>
                                                <div className="flex items-center gap-2.5 text-gray-900 font-bold mb-1.5">
                                                    ...
                                                    <span>{review.orders?.users?.fullname || 'Ẩn danh'}</span>
                                                </div>
                                                <div className="text-xs text-gray-500 pl-6.5 truncate">
                                                    {review.orders?.users?.email || 'Guest'}
                                                </div>
                                            </div>
                                            ...
                                            <div>
                                                <div className="flex items-center gap-2.5 text-gray-700 font-semibold mb-1.5 text-sm">
                                                    ...
                                                    <span className="text-[#ee4d2d]">#{review.orderid?.substring(0, 8).toUpperCase()}</span>
                                                </div>
                                                <div className="flex items-center gap-2.5 text-[10px] text-gray-400 pl-6.5 font-medium">
                                                    <Calendar className="w-3.5 h-3.5" />
                                                    {new Date(review.orders?.ordertime || review.createdat).toLocaleDateString("vi-VN")}
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex-1">
                                            <div className="flex items-center justify-between mb-5">
                                                ...
                                                <span className="text-xs text-gray-400 font-medium italic">
                                                    Gửi lúc: {new Date(review.createdat).toLocaleString("vi-VN")}
                                                </span>
                                            </div>
                                            ...
                                            {review.type === 'food' && review.fooditems && (
                                                <div className="flex items-center gap-4 mb-5 p-4 bg-gray-50 rounded-2xl border border-gray-100 shadow-inner">
                                                    ...
                                                    <div>
                                                        <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wide">Sản phẩm đánh giá</p>
                                                        <p className="text-base font-bold text-gray-900">{review.fooditems.foodname}</p>
                                                    </div>
                                                </div>
                                            )}
