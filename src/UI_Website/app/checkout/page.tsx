...
export default function CheckoutPage() {
    ...
    const finalBillAmount = totalGoodsAmount + fixedShippingFee;
    ...
    function submitOrderToStore() {
        if (!deliveryAddress || !contactPhoneNumber) {
            alert("Vui lòng nhập đầy đủ địa chỉ và số điện thoại để chúng tôi giao hàng!");
            return;
        }
    ...
    if (wasOrderSuccessful === true) {
        return (
            <div className="min-h-screen bg-white flex flex-col items-center justify-center p-6 text-center">
                ...
                <h1 className="text-4xl font-extrabold text-gray-900 mb-4">Đặt đơn hàng thành công!</h1>
                <p className="text-gray-500 mb-10 max-w-lg leading-relaxed text-lg">
                    Cảm ơn bạn đã tin dùng ShopeeFood. Đơn hàng <span className="font-bold text-[#ee4d2d]">{generatedOrderCode}</span> đang được xử lý và sẽ sớm được tài xế giao đến bạn.
                </p>
                <div className="flex flex-col sm:flex-row gap-4 w-full max-w-md">
                    <Button
                        className="flex-1 bg-[#ee4d2d] hover:bg-[#d73211] py-8 text-xl font-bold rounded-2xl shadow-xl shadow-orange-100 transition-all active:scale-95"
                        onClick={function () { pageRouter.push("/"); }}
                    >
                        Tiếp tục mua sắm
                    </Button>
                </div>
        ...
        return (
            <div className="min-h-screen bg-gray-50 flex flex-col">
                ...
                <div className="container mx-auto py-32 px-4 text-center">
                    ...
                    <h2 className="text-3xl font-bold mb-4 text-gray-800">Giỏ hàng của bạn đang trống</h2>
                    <p className="text-gray-500 mb-10">Hãy quay lại trang chủ để chọn những món ăn ngon nhé!</p>
                    <Button
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        <div className="bg-white p-8 rounded-3xl shadow-sm border border-orange-100/50">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2.5">
                                    <Label htmlFor="contactPhone" className="font-bold text-gray-700">Số điện thoại</Label>
                                    <div className="relative">
                                        ...
                                        <Input
                                            ...
                                            onChange={function (e) { setContactPhoneNumber(e.target.value); }}
                                        />
                                    </div>
                                </div>
                                <div className="space-y-2.5">
                                    <Label htmlFor="deliveryAddr" className="font-bold text-gray-700">Địa chỉ chi tiết</Label>
                                    <div className="relative">
                        ...
                        <div className="bg-white rounded-3xl overflow-hidden shadow-sm border border-gray-100">
                            ...
                            <div className="divide-y divide-gray-50 px-4 md:px-0">
                                {itemsToPurchase.map(function (item) {
                                    return (
                                        <div key={item.id} className="p-6 flex flex-col md:flex-row items-center justify-between gap-4 md:gap-0 hover:bg-gray-50/50 transition-colors">
                                            ...
                                            <div className="flex items-center justify-between w-full md:w-auto md:gap-16 text-sm">
                                                <p className="md:w-20 text-right font-medium text-gray-500">{item.price}</p>
                                                <p className="md:w-16 text-center font-bold text-gray-900">x{item.quantity}</p>
                                                <p className="md:w-24 text-right font-black text-[#ee4d2d] text-base">
                                                    {formatVNDPrice(parsePrice(item.price) * item.quantity)}
                                                </p>
                                            </div>
                                        </div>
                                    );
                        ...
                        <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
                            <h2 className="text-xl font-black mb-6 text-gray-900 border-l-4 border-[#ee4d2d] pl-4">Phương thức thanh toán</h2>
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                <button
                                    className={`flex flex-col items-center justify-center p-6 border-2 rounded-2xl transition-all duration-300 ${selectedPaymentMethod === "shopeepay" ? "border-[#ee4d2d] bg-orange-50 text-[#ee4d2d] shadow-md shadow-orange-100" : "border-gray-50 hover:border-gray-100 text-gray-400"}`}
                                    onClick={function () { setSelectedPaymentMethod("shopeepay"); }}
                                >
                                    <Smartphone className="w-8 h-8 mb-3" />
                                    <span className="text-sm font-bold">Ví ShopeePay</span>
                                </button>
                                <button
                                    ...
                                    onClick={function () { setSelectedPaymentMethod("cash"); }}
                                >
                                ...
                                <button
                                    ...
                                    onClick={function () { setSelectedPaymentMethod("bank"); }}
                                >
                                ...
                                <div className="mt-8 p-8 border-2 border-dashed border-orange-200 rounded-3xl bg-orange-50/30 flex flex-col items-center animate-in fade-in zoom-in duration-500">
                                    ...
                                    <div className="text-center space-y-4 mb-10 bg-white p-6 rounded-2xl w-full max-w-sm shadow-sm border border-orange-100">
                                        <div className="flex justify-between items-center text-gray-500">
                                            ...
                                            <span className="font-black text-gray-900 text-lg uppercase tracking-widest">{generatedOrderCode}</span>
                                        </div>
                                        <div className="flex justify-between items-center border-t border-gray-50 pt-4">
                                            <span className="text-sm font-medium">Số tiền:</span>
                                            <span className="font-black text-[#ee4d2d] text-2xl">{formatVNDPrice(finalBillAmount)}</span>
                                        </div>
                                    </div>
                                    <Button
                                        className="bg-green-600 hover:bg-green-700 text-white font-black px-16 py-8 rounded-2xl shadow-xl shadow-green-100 transition-all hover:scale-105 flex items-center gap-3 text-xl"
                    ...
                    <div className="lg:col-span-1 space-y-6">
                        ...
                        <div className="bg-white p-8 rounded-3xl shadow-lg border border-gray-100 sticky top-28">
                            <h2 className="text-2xl font-black mb-8 text-gray-900">Chi tiết thanh toán</h2>
                            ...
                            <div className="space-y-6 mb-8">
                                <div className="flex justify-between text-gray-500 font-bold mb-6">
                                    ...
                                    {voucherCode ? (
                                        ...
                                        <button
                                            ...
                                            onClick={function () { setVoucherCode("DUMMYFREE"); }}
                                        >
                                            Chọn voucher
                                        </button>
                                    )}
                                ...
                                <div className="flex flex-col gap-2 pb-6 border-b border-gray-50">
                                    <div className="flex justify-between text-gray-500 font-medium">
                                        ...
                                        <span className="text-gray-900">{formatVNDPrice(totalGoodsAmount)}</span>
                                    </div>
                                    <div className="flex justify-between text-gray-500 font-medium">
                                        <span>Phí vận chuyển:</span>
                                        <span className="text-gray-900">{formatVNDPrice(fixedShippingFee)}</span>
                                    </div>
                                    <div className="flex justify-between text-gray-500 font-medium pt-2">
                                        <div className="flex items-center gap-2">
                                ...
                                <div className="flex justify-between text-3xl font-black text-[#ee4d2d] pt-2">
                                    <span className="text-gray-900 text-lg flex items-center">Tổng:</span>
                            ...
                            <div className="mt-8 space-y-4 pt-6 border-t border-gray-50">
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-xs font-black text-gray-400 uppercase tracking-tighter">Ghi chú cho quán:</span>
                                </div>
                                <Input
                                    placeholder="Ví dụ: Đồ ăn nhớ cho cay ít thôi nha..."
                                    className="bg-gray-50 border-none rounded-xl h-12 text-sm italic"
                                    value={customerNote}
                                    onChange={function (e) { setCustomerNote(e.target.value); }}
                                />
                            </div>
                        </div>
                    </div>
