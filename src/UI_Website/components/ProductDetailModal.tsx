...
export default function ProductDetailModal({ product, isOpen, onClose }: ProductDetailModalProps) {
    ...
    const handleToppingToggle = (topping: ToppingOption) => {
        setSelectedToppings(prev => {
            const isSelected = prev.some(t => t.toppingName === topping.toppingName);
            if (isSelected) {
    ...
    const calculateTotalPrice = () => {
        ...
        const toppingPrice = selectedToppings.reduce((sum, t) => sum + parsePrice(t.price), 0);
                        ...
                        {product.toppingOptions && product.toppingOptions.length > 0 && (
                            <div className="space-y-4">
                                <h4 className="text-sm font-black text-gray-900 uppercase tracking-widest flex items-center gap-2">
                                    Thêm Topping
                                    <span className="text-[10px] bg-gray-100 text-gray-400 px-2 py-0.5 rounded-full font-bold">TÙY CHỌN</span>
                                </h4>
                                <div className="space-y-2">
                                    {product.toppingOptions.map((topping) => {
                                        const isSelected = !!selectedToppings.find(t => t.toppingName === topping.toppingName);
                                        return (
                                            <div
                                                key={topping.toppingName}
                                                className={`flex items-center justify-between p-4 rounded-2xl border transition-all cursor-pointer group ${isSelected ? 'border-[#ee4d2d]/30 bg-orange-50/50' : 'border-gray-100 hover:bg-gray-50'
                                                    }`}
                                                onClick={() => handleToppingToggle(topping)}
                                            >
                                                <div className="flex items-center gap-3">
                                                    <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${isSelected ? 'bg-[#ee4d2d] border-[#ee4d2d]' : 'border-gray-200 bg-white'
                                                        }`}>
                                                        {isSelected && <div className="w-2 h-2 bg-white rounded-full" />}
                                                    </div>
                                                    <span className={`text-sm font-bold transition-colors ${isSelected ? 'text-[#ee4d2d]' : 'text-gray-700'}`}>
                                                        {topping.toppingName}
                                                    </span>
                                                </div>
                        ...
                        <div className="mt-4 pt-6 border-t border-gray-100 space-y-6">
                            <div className="flex items-center justify-between">
                                <div className="flex flex-col">
                                    <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Số lượng</span>
                                    <div className="flex items-center gap-4 bg-gray-50 p-1.5 rounded-2xl border border-gray-100">
                                        ...
                                        <span className="w-6 text-center font-black text-lg text-gray-900">{quantity}</span>
                                        <Button
                                            variant="ghost"
                                            size="icon"
                                            className="h-8 w-8 rounded-xl hover:bg-white hover:text-[#ee4d2d] shadow-sm transition-all"
                                ...
                                <div className="flex flex-col items-end">
                                    <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Tổng cộng</span>
